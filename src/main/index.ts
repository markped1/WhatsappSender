...
if (!isLock) {
  ...
  app.whenReady().then(async () => {
    ...
    // Pre-initialize WhatsApp in background so QR is ready when user clicks LINK
    whatsappClient.initialize().catch(e => console.error('[WA] Pre-init failed:', e.message))
    ...
    ipcMain.handle('whatsapp:logout', () => whatsappClient.logout())
    ...
    ipcMain.handle('license:activate', (_event, key) => activateKey(key))
